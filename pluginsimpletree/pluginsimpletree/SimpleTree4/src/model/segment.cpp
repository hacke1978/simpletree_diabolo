...
QVector<QSharedPointer<Cylinder> > Segment::get_cylinders() const
{
    return _cylinders;
}
...
QSharedPointer<PointI> Segment::get_start() const
{
    assert(!_cylinders.isEmpty());
    QSharedPointer<Cylinder> cylinder = _cylinders.first();
    return cylinder->get_start();
}

QSharedPointer<PointI> Segment::get_end() const
{
    assert(!_cylinders.isEmpty());
    QSharedPointer<Cylinder> cylinder = _cylinders.last();
    return cylinder->get_end();
}
