...
ST_StepCropQSM::ST_StepCropQSM(CT_StepInitializeData &dataInit) : CT_AbstractStep(dataInit)
{
    _min_rad = 0.035f;
}

ST_StepCropQSM::~ST_StepCropQSM()
{
}


// Step description (tooltip of contextual menu)
QString ST_StepCropQSM::getStepDescription() const
{
    return tr("QSM spherefollowing method - crops everything smaller than a certain diameter.");
}

// Step detailled description
QString ST_StepCropQSM::getStepDetailledDescription() const
{
    return tr("QSM spherefollowing method - crops everything smaller than a certain diameter.");
}

// Step URL
QString ST_StepCropQSM::getStepURL() const
{
...
// Step copy method
ST_StepCropQSM* ST_StepCropQSM::createNewInstance(CT_StepInitializeData &dataInit)
{
    return new ST_StepCropQSM(dataInit);
}
...
void ST_StepCropQSM::createPostConfigurationDialog()
{
    CT_StepConfigurableDialog *configDialog = newStandardPostConfigurationDialog();


    configDialog->addText(tr("This step crops removes from a QSM everything from."));
    configDialog->addBool(tr("the tips up to a certain diameter"),"","",_redo_pipe);
    configDialog->addDouble(tr("A minimum radius up to which the tree model is cropped."),"",0,4, 4,_min_rad);
...
void ST_StepCropQSM::createInResultModelListProtected()
{
...
void ST_StepCropQSM::createOutResultModelListProtected()
{
...
void ST_StepCropQSM::add_cylinder_data(QSharedPointer<Tree> tree, CT_ResultGroup *resCpy_res, CT_StandardItemGroup *grpCpy_grp)
{
...
void ST_StepCropQSM::compute()
{
    ...
    radii.push_back(0.025);
    radii.push_back(0.03);
    radii.push_back(0.035);
    radii.push_back(0.04);
    radii.push_back(0.045);
    radii.push_back(0.05);
    radii.push_back(0.055);
    radii.push_back(0.06);
    radii.push_back(0.065);
    ...
    while (itCpy_grp.hasNext() && !isStopped())
    {
        ...
        if(tree_in !=0)
        {
            ...
            QVectorIterator<float> git(radii);
            while(git.hasNext())
            {
                ...
                if(tree_old!=0)
                {
                    ...
                    float radius = git.next();
                    QString extension = QString::number(radius * 1000);
                    file_id.append("-");
                    file_id.append(extension);
                                        file_id.append("-");
                    coeff.id = file_id;
                    ...
                    QString path = "D:/test/crop_new";
                                         Export::export_tree_detail(tree_clone,path, file_name,coeff);
