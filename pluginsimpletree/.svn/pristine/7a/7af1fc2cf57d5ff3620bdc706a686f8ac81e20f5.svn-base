#include "st_stepcompletemodelling.h"
...
ST_StepCompleteModelling::ST_StepCompleteModelling(CT_StepInitializeData &dataInit) : CT_AbstractStep(dataInit)
{
    _knn = 4;
...
ST_StepCompleteModelling::~ST_StepCompleteModelling()
{
    _sceneList->clear();
    delete _sceneList;
}
...
QString ST_StepCompleteModelling::getStepDescription() const
{
    return tr("Complete Modelling.");
}

// Step detailled description
QString ST_StepCompleteModelling::getStepDetailledDescription() const
{
    return tr("See for now SimpleTree homepage." );
}

// Step URL
QString ST_StepCompleteModelling::getStepURL() const
{
...
// Step copy method
CT_VirtualAbstractStep* ST_StepCompleteModelling::createNewInstance(CT_StepInitializeData &dataInit)
{
    return new ST_StepCompleteModelling(dataInit);
}

//////////////////// PROTECTED METHODS //////////////////

// Creation and affiliation of IN models
void ST_StepCompleteModelling::createInResultModelListProtected()
{
...
void ST_StepCompleteModelling::createOutResultModelListProtected()
{
...
void ST_StepCompleteModelling::createPostConfigurationDialog()
{
    ...
    //    configDialog->addDouble(tr("Min radius sphere"), "m", 0, 100, 3, _minRadiusSphere, 1);


    configDialog->addEmpty();
    configDialog->addTitle(tr("<b>Allometry parameters</b>"));
    configDialog->addDouble(tr("a"), "", 0, 100000, 4, _a, 1);
    configDialog->addDouble(tr("b"), "", 0, 100000, 4, _b, 1);
...
void ST_StepCompleteModelling::compute()
{
...
void ST_StepCompleteModelling::add_cylinder_data(Tree tree, CT_ResultGroup *resCpy_res, CT_StandardItemGroup *grpCpy_grp, QString string)
{
    ...
    while(it.hasNext()) {
        ...
        //             qDebug() << "in loop before bla";
        if (!(cylin->get_segment()->is_root()))
        {
            parentSegmentID = cylin->get_segment()->get_parent_segment()->get_id();
        }
        //       qDebug() << "in loop before";
        float growthVolume = tree.get_growth_volume(cylin);

        int detection = cylin->get_detection();
        //        if(cylin->get_detection()==0)
        //        {
        //            detection = "SPHEREFOLLOWING";
        //        } else if (cylin->get_detection()==1)
        //        {
        //            detection = "ATTRACTOR";
        //        }
        int improvement = cylin->get_improvement();

        //        if(cylin->get_improvement()==0)
        //        {
        //            improvement = "RANSAC";
        //        } else if (cylin->get_improvement()==1)
        //        {
        //            improvement = "MEDIAN";
        //        } else if (cylin->get_improvement()==2)
        //        {
        //            improvement = "NO";
        //        }
        // qDebug() << "in loop after";
...
CT_TTreeGroup *ST_StepCompleteModelling::constructTopology(const CT_AbstractResult *res_r, QSharedPointer<Tree> tree, QString string)
{
...
void ST_StepCompleteModelling::constructTopologyRecursively(const CT_AbstractResult *res_r, CT_TNodeGroup *parent, QSharedPointer<Segment> segment, QSharedPointer<Tree> tree, QString string)
{
...
void ST_StepCompleteModelling::setCylinders(const CT_AbstractResult *res_r, CT_TNodeGroup *root, QSharedPointer<Segment> segment, QSharedPointer<Tree> tree, QString string)
{
...
void ST_StepCompleteModelling::build_tree_model(CT_ResultGroup *resCpy_res, CT_StandardItemGroup *grpCpy_grp)
{
    ...
    //Tree tree (builder.getRoot_segment());
    ...
    //ImproveBranchJunctions improve_branch_junctions(tree);
    ...
    ReorderTree reorder3(tree);
   // qDebug() << "allom 0 " << coeff.a <<  "," <<coeff.b;


    //qDebug() << "allom a " << coeff.a <<  "," <<coeff.b;
...
void ST_StepCompleteModelling::enrich_cloud(const CT_AbstractItemDrawableWithPointCloud *itemCpy_cloud_in, CT_ResultGroup* resCpy_res, CT_StandardItemGroup* grpCpy_grp)
{
...
void ST_StepCompleteModelling::create_simple_tree_cloud(const CT_AbstractItemDrawableWithPointCloud *itemCpy_cloud_in)
{
